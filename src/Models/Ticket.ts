...
TicketSchema.static('createTicket', async (context: Context): Promise<TextChannel> => {
    ...
    const categoryModel = await VariableModel.findOne({ key: 'ticket_category' });
    if (!categoryModel) return;

    // Get staff role id for tickets
    const staffRoleModel = await VariableModel.findOne({ key: 'staff_role' });
    if (!staffRoleModel) return;
    
    // Create channel
    const channel: TextChannel = await context.guild.channels.create(ticketId, {
        type: 'GUILD_TEXT',
        parent: categoryModel.value,
        permissionOverwrites: [
            {
                id: staffRoleModel.value,
                type: 'role',
    ...
    collector.on('end', async data => {
        ...
        channel.send({ content: `<@&${staffRoleModel.value}>` }).then(resp => context.client.cleanUp(250, resp));
    });
    return channel;
});
